...
### Phase 1.5: UI・UXブラッシュアップ（現在ここ）
- キャラクター画像の実装（ユーザー提供画像を `client/public/assets/characters/` に配置想定）
- キャラクター選択画面のレイアウト改善（縦並びからグリッド表示など見やすく）
- バトル画面の技ボタンレイアウトの改善（操作ミスの軽減と視認性向上）
- バトル画面での各キャラクターの「パッシブ能力」の常時表示
- キャラクター選択画面でのホバーによる詳細性能（ステータス・技）表示
- 初心者向けのゲームルール・相性表マニュアル（Help画面）の実装
